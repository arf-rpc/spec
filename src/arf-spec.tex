...
FNV-1a-32(input):
  ...
  \label{sec:requests}
    ...
    \begin{itemize}
      \item If \textbf{HasInput} is \texttt{true}, the \texttt{INVOKE} payload
        MUST contain the serialization of the unary input \emph{tuple},
        consisting of the ordered sequence of unary input values as declared
        in the method signature.
      \item If \textbf{HasInput} is \texttt{false}, the payload MUST be empty.
      \item If \textbf{HasInputStream} is \texttt{true}, the input stream is
        ...
        If the RPC is aborted by sending \texttt{CANCEL}, the client MUST
        NOT send \texttt{IN\_CLOSE}. Cancellation implicitly closes the
        input stream.
    \end{itemize}
  ...
  \subsection{Interleaving of Input and Output Streams}
    ...
    A server MAY transmit \texttt{RESPONSE} before, during, or after any
    stream activity. No ordering relationship is implied unless explicitly
    stated by a method’s application semantics.
  ...
  \subsection{Unary vs Streaming Semantics}
    ...
    \subsubsection{Unary Input}
      ...
      serialized unary input tuple.  If false, the payload MUST be empty.
    ...
    \subsubsection{Input Stream}
      ...
      \begin{itemize}
        ...
        \item If \textbf{HasInputStream} is \texttt{true} and the RPC
          completes normally, the client MUST send exactly one
          \texttt{IN\_CLOSE} when the RPC completes normally.

          If the RPC is terminated by \texttt{CANCEL} or \texttt{ERROR}, the
          client MUST NOT send \texttt{IN\_CLOSE}. Cancellation or error
          implicitly closes the input stream.
    ...
    \label{sec:unary-output}
      ...
      \texttt{RESPONSE} frame MUST contain the serialized unary output tuple,
      encoded as specified in Section~\ref{sec:unary-tuples}, using the unary
      output values in their declaration order.  If \textbf{HasOutput} is
      \texttt{false}, the payload of the \texttt{RESPONSE} frame MUST be
      empty.
