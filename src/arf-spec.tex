  ...
  \subsubsection{Terms}
    ...
    \begin{description}
      ...
      \item[Form] One of the sixteen method signature combinations
        defined in Section~\ref{sec:method-forms}, derived from the
...
v1beta1.mypackage.OtherName
  ...
  \label{sec:types}
    ...
    \begin{itemize}
      ...
      \item \texttt{timestamp}: represents an instant on the UTC timeline,
        encoded as a signed integer count of milliseconds elapsed since the Unix
        Epoch (\texttt{1970-01-01T00:00:00Z}).  Values prior to the Unix Epoch
        are represented as negative integers and are encoded using the signed
        integer encoding rules defined in Section~\ref{sec:signed-int-encoding}.
  ...
  \label{sec:enums}
    ...
    \label{sec:enum-encoding}
      ...
      Decoders MUST accept any enum value whose discriminant does not match a
      known case in the enum definition and MUST preserve its numeric
      discriminant value.  Applications that require closed-set semantics SHOULD
      explicitly validate that a received discriminant is one of the known cases
      and reject or handle unknown values according to application policy.

      This loose validation is intentional to allow enum definitions to evolve
      without breaking older receivers, while still enabling applications to
      enforce stricter semantics when required.  In any case, discriminants MUST
      NOT exceed 65535 prior to \texttt{VarUInt} encoding.
    ...
    \subsubsection{Evolution Rules}
      ...
      Although enum declarations define a finite set of known values at a given
      version, receivers MUST be prepared to handle unknown discriminants as
      defined in Section~\ref{sec:enum-encoding}.
...
@bar("a", "b")
  ...
  \subsubsection{Methods}
    ...
    However, certain combinations are intentionally forbidden because they tend
    to produce fragile or awkward generated APIs.

    Methods MUST NOT declare unary output values together with an output stream.
    Thus, any method whose form has \textbf{HasOutput}=\texttt{true} and
    \textbf{HasOutputStream}=\texttt{true} (i.e., NYNY, NYYY, YYNY, YYYY) is
    invalid and MUST be rejected by compilers.

    Additionally, methods MUST NOT declare unary output values together with an
    input stream. Thus, any method whose form has \textbf{HasOutput}=\texttt{true}
    and \textbf{HasInputStream}=\texttt{true} (i.e., NYYN and YYYN) is invalid
    and MUST be rejected by compilers.

    This restriction avoids APIs where the response is only available after the
    client half-closes the input stream, which tends to leak protocol sequencing
    into application logic.

    The remaining ten forms are considered \emph{legal method forms}.  Each
...
\]
    ...
    \subsubsection{Width Enforcement}
      ...
      Decoders MUST reject any decoded value whose magnitude exceeds the range
      of the declared signed type:
      ...
      Encoders MUST NOT emit values outside the representable range of the
      declared type.
...
\]
  ...
  \subsection{Primitive Types}
    ...
    \begin{itemize}
      ...
      \item \textbf{timestamp}: represents an instant on the UTC timeline,
        encoded as a signed 64-bit integer count of milliseconds elapsed since
        the Unix Epoch (\texttt{1970-01-01T00:00:00Z}).  Values prior to the
        Unix Epoch are represented as negative integers and are encoded using
        the signed integer encoding rules defined in
        Section~\ref{sec:signed-int-encoding}.

        Timestamp values represent absolute instants and do not carry timezone
        or offset information. Implementations that cannot represent millisecond
        precision MUST truncate toward zero.
    \end{itemize}
    ...
    \textbf{Note:} Primitive types defined in this section are forbidden as
    direct RPC inputs, outputs, or stream elements, as specified in
    Section~\ref{sec:methods}.
  ...
  \subsection{Composite Types}
    ...
    \subsubsection{Maps}
      ...
      ordering or sorting. The preserved order has no semantic meaning and MUST
      NOT be relied upon by applications.
    ...
    \subsubsection{Structs}
      ...
      struct instance is preceded by a length indicator that defines the total
      ...
      \begin{enumerate}
        ...
        \item For a field whose declared type is not \texttt{optional<\dots>}:
          ...
          the rules for its type.  Encoders MUST NOT omit fields;
          decoders MUST treat truncation before the end of a required
          field as an error.
      \end{enumerate}

      Decoders MUST NOT assume knowledge of the physical layout of a
      struct beyond the fields they explicitly recognize.  After decoding the
      fields they understand, implementations MUST skip any remaining
      ...
      MUST NOT reorder fields within a package version.
...
\label{sec:rpc-protocol}
  ...
  All frames are self-contained and do not rely on lower-layer segmentation
  semantics.

  Unless otherwise noted, all multi-byte integer fields not being governed by
  VarUInt or Zig-Zag encoding are encoded in big-endian byte order.  This also
  applies to float values.
  ...
  \label{sec:frame-format}
    ...
    \subsubsection{INVOKE frame}
      ...
      the \texttt{INVOKE} frame must contain an empty unary input tuple; meaning
      that only a \texttt{VarUInt(0)} is present.
      ...
      \begin{itemize}
        ...
        \item \texttt{Payload}: when \textbf{HasInput} is \texttt{true}, the
          ...
          is \texttt{false}, an empty unary input tuple MUST be encoded as
          VarUInt(0), while other fields MUST be present.
      \end{itemize}
    ...
    \subsubsection{RESPONSE Frame}
      ...
      The \texttt{RESPONSE} frame is the terminal positive output of an RPC.

      The payload of an \texttt{RESPONSE} frame MUST be a \emph{unary output
      tuple}.  Even for frames containing no items, the first portion of the
      \emph{unary output tuple} MUST be present, indicating a zero-length tuple,
      as specified in Section~\ref{sec:unary-tuples}.  For methods with unary
      outputs, the unary output tuple is encoded as the total number of bytes
      occupied by the concatenation of the serialized values comprising the
      tuple through an VarUInt, followed by the serialized values of each of
      them.
  ...
  \label{sec:multiplexing}
    ...
    \subsubsection{Connection-Level Protocol Violations}
      Violations that prevent reliable correlation or recovery (e.g., duplicate
      \texttt{INVOKE}, invalid frame sequencing) MUST result in transport
      closure.  Other violations MAY be reported via \texttt{ERROR} when safe to
      do so.
...
static inline int64_t arf_zigzag64_decode(uint64_t z) {
  ...
  \label{annex:testvectors}
    ...
    The following test vectors are normative.  Implementations MUST
