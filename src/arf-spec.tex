...
v1beta1.mypackage.OtherName
  ...
  \label{sec:types}
    ...
    or stream element types. Only user-defined \texttt{struct} and \texttt{enum}
    types are permitted in RPC method signatures (see Section~\ref{sec:methods}).
...
@bar("a", "b")
  ...
  \subsubsection{Methods}
    ...
    However, arf forbids method signatures that declare both unary output values
    and an output stream. Any method whose form has \textbf{HasOutput} =
    \texttt{true} and \textbf{HasOutputStream} = \texttt{true} (i.e., forms
    NYNY, NYYY, YYNY, and YYYY) is invalid and MUST be rejected by compilers.
  ...
  \subsubsection{IDL Syntax for Streams}
    ...
    A method that declares an output stream MUST NOT declare any unary output
    ...
    arf and MUST be rejected by compilers.
...
\label{sec:rpc-protocol}
  ...
  on lower-layer segmentation semantics.
    ...
    \label{sec:frame-format}
      ...
      well-formed sequence of complete frames.  The format of a frame is:
    ...
    \subsubsection{INVOKE frame}
      ...
      identifiers, along with the Metadata block and, when \textbf{HasInput} is
      \texttt{true}, the unary input tuple required by the method. When
      \textbf{HasInput} is \texttt{false}, the \texttt{INVOKE} frame must omit
      the input tuple.
      ...
      \begin{itemize}
        ...
        \item \texttt{MethodID}: the identifier of the method being called on
          ...
          integer.
        \item \texttt{Metadata}: a metadata block as described in
        ...
        \item \texttt{Payload}: when \textbf{HasInput} is \texttt{true}, the
          ...
          omitted, while other fields MUST be present.
      \end{itemize}
    ...
    \subsubsection{IN\_STREAM Frame}
      ...
      after receiving \texttt{CONTINUE}.
      ...
      \texttt{IN\_CLOSE}, nor after sending \texttt{CANCEL}. Additionally,
      \texttt{IN\_STREAM} frames MUST NOT contain Metadata blocks.
    ...
    \subsubsection{IN\_CLOSE Frame}
      ...
      Sending \texttt{IN\_CLOSE} when the method does not declare an input stream
      is a protocol error.
    ...
    \subsubsection{OUT\_STREAM Frame}
      ...
      \texttt{OUT\_CLOSE} or after sending \texttt{ERROR}. Additionally,
      \texttt{OUT\_STREAM} frames MUST NOT contain Metadata blocks.

    \subsubsection{OUT\_CLOSE Frame}

      The \texttt{OUT\_CLOSE} frame signifies that the server has sent all output
      stream elements.

      A \texttt{OUT\_CLOSE} frame MUST contain an empty payload.

      Sending \texttt{OUT\_CLOSE} when the method does not declare an output
      stream is a protocol error.

    \subsubsection{RESPONSE Frame}

      The \texttt{RESPONSE} frame terminates the RPC returning its unary output
      portion.

      The payload of a \texttt{RESPONSE} frame consists of:

      \begin{itemize}
        \item a metadata block; and
        \item if \textbf{HasOutput} is \texttt{true}, a unary output tuple as
          defined in Section~\ref{sec:unary-tuples}; otherwise the payload MUST
          contain no tuple data.
      \end{itemize}

      If a method declares output streams, the server MUST send
      \texttt{OUT\_CLOSE} before sending \texttt{RESPONSE}.
    ...
    \subsubsection{ERROR Frame}
      ...
      \texttt{ERROR} frames MUST NOT contain a Metadata block.

  \subsection{Requests}
  \label{sec:requests}

    An RPC request is initiated by the client sending an \texttt{INVOKE} frame.
    The on-wire layout of \texttt{INVOKE} (identifiers, metadata, and optional
    unary inputs) is defined in Section~\ref{sec:frame-format} and its frame
    subsections. This subsection specifies only sequencing and lifecycle rules.
    ...
    \begin{itemize}
      ...
      \item If \textbf{HasInputStream} is \texttt{true}, the input stream is
        considered open after \texttt{CONTINUE}. The client MAY send zero or more
        \texttt{IN\_STREAM} frames. For a successfully completing RPC (no
        \texttt{CANCEL} or \texttt{ERROR}), the client MUST send exactly one
        \texttt{IN\_CLOSE}, and the server MUST NOT send \texttt{RESPONSE} until it
        has received that \texttt{IN\_CLOSE}. After \texttt{CANCEL}, the client
        MUST NOT send any further \texttt{IN\_STREAM} or \texttt{IN\_CLOSE} frames
        for that \texttt{CorrelationID}.
      \item A client MUST NOT send another \texttt{INVOKE} with the same
        ...
        receives a duplicate \texttt{INVOKE} for an active RPC, it MUST treat this
        as a connection-level protocol error and MUST close the transport
  ...
  \label{sec:identifiers}
    ...
    \subsubsection{FNV-1a Hash Function}
        ...
        arf derives \texttt{PackageID}, \texttt{ServiceID}, and
        \texttt{MethodID} values using the FNV-1a non-cryptographic hash
        function operating on 32-bit unsigned integers.  FNV-1a is selected for
        its simplicity, determinism, efficiency, and well-defined behavior
        ...
        Implementations MUST treat the input byte stream as an ordered sequence
        of octets.  No Unicode normalization, case-folding, or locale-sensitive
...
FNV-1a-32(input):
  ...
  \label{RPC-MethodForms}
    ...
    \begin{table}[H]
      ...
      \vskip 0.5em
      \noindent
      Forms in which \textbf{HasOut} and \textbf{HasOutS} are both \texttt{Y}
      (namely NYNY, NYYY, YYNY, and YYYY) are reserved and \emph{MUST NOT} be
      used in arf method declarations. Compilers MUST reject any method whose
      signature would result in one of these forms. All other forms in
      Table~\ref{tab:method-forms} are legal.
    \end{table}
    ...
    \begin{itemize}
      ...
      \item If HasInS is \texttt{Y} and the RPC initializes successfully, the
        ...
        client MUST send exactly one \texttt{IN\_CLOSE} for that
        \texttt{CorrelationID}, and the server MUST NOT send \texttt{RESPONSE}
        until it has received that \texttt{IN\_CLOSE}.

        If the RPC is terminated by \texttt{CANCEL} or \texttt{ERROR},
        \texttt{IN\_CLOSE} MUST NOT be sent. If HasInS is \texttt{N}, the
        client MUST NOT send \texttt{IN\_STREAM} or \texttt{IN\_CLOSE} at any
        time.
      ...
      \item If HasOutS is \texttt{Y} and the RPC is initialized successfully,
        ...
        implicitly closes the output stream. If HasOutS is \texttt{N}, the
        server MUST NOT send \texttt{OUT\_STREAM} or \texttt{OUT\_CLOSE} at
        any time.
    \end{itemize}
  ...
  \subsection{Unary vs Streaming Semantics}
    ...
    \label{sec:unary-input}
      ...
      The \texttt{INVOKE} frame layout (identifiers, metadata, and optional unary
      inputs) is defined in Section~\ref{sec:frame-format}. This subsection states
      only when the unary input tuple is present: if \textbf{HasInput} is
      \texttt{true}, the unary input tuple is included after the metadata block; if
      \textbf{HasInput} is \texttt{false}, no unary input tuple is encoded. Clients
      MUST NOT encode a unary input tuple when \textbf{HasInput} is \texttt{false}.
    ...
    \label{sec:rpc-completion}
      ...
      \texttt{CANCELLED} indicates that the RPC was cancelled by the client. The
      semantics of cancellation and how methods should respond to a cancellation
      signal is implementation-dependent and out of scope of this document.
      ...
      \begin{itemize}
        ...
        \item The endpoint has observed either:
          \begin{itemize}
            \item successful unary completion through a \texttt{RESPONSE} frame; or
    ...
    \label{sec:unary-output}
      ...
      \begin{itemize}
        \item If \textbf{HasOutput} is \texttt{true}, the \texttt{RESPONSE}
          payload contains the unary output tuple after the metadata block;
          otherwise, it contains only the metadata block. In all legal method
          forms, successful completion is signalled by exactly one
        ...
        \item If \textbf{HasOutputStream} is \texttt{true}, exactly one
          ...
          \texttt{RESPONSE}.
        \item If \textbf{HasInputStream} is \texttt{true}, the server MUST NOT send
          \texttt{RESPONSE} until it has received exactly one \texttt{IN\_CLOSE} for
...
INVOKE
CONTINUE
RESPONSE   // metadata + unary output tuple
      \end{lstlisting}
...
struct RPCError {
    ...
    \label{sec:cancellation}
      ...
      A \texttt{CANCEL} frame MUST carry an empty payload;
      its \texttt{PayloadLength} field MUST be zero.
...
\label{sec:evolution}
  ...
  \subsection{Removing Fields}
    ...
    accept and ignore deprecated fields if they appear.
