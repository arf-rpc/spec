...
\label{sec:rpc-protocol}
  ...
  on lower-layer segmentation semantics.
    ...
    \subsubsection{INVOKE frame}
      ...
      \textbf{HasInput} is \texttt{false}, the \texttt{INVOKE} payload MUST
      be empty.
      ...
      The \texttt{INVOKE} payload is composed of the following items, in order:
      ...
      \begin{itemize}
        ...
        \item \texttt{Payload}: when \textbf{HasInput} is \texttt{true}, the
          ...
          is \texttt{false}, no tuple is encoded and the payload MUST be empty.
      \end{itemize}
    ...
    \subsubsection{OUT\_CLOSE Frame}
      ...
      If \textbf{HasOutputStream} is \texttt{true}, the server MUST send exactly
      one \texttt{OUT\_CLOSE} frame before the \texttt{RESPONSE} frame.
...
FNV-1a-32(input):
  ...
  \subsection{Unary vs Streaming Semantics}
    ...
    \label{sec:unary-output}
      ...
      Section~\ref{sec:frame-format}. Every RPC that completes successfully (no
      \texttt{ERROR}) MUST send exactly one \texttt{RESPONSE} for the corresponding
      \texttt{CorrelationID}, and it MUST be the last application-level frame for
      that RPC. All ordering and closure rules remain unchanged:

      \begin{itemize}
        \item If \textbf{HasOutput} is \texttt{true}, the \texttt{RESPONSE} payload
          contains the unary output tuple after the metadata block; otherwise, it
          contains only the metadata block.
