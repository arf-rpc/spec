...
v1beta1.mypackage.OtherName
  ...
  \subsection{Types}
    ...
    \texttt{VarUInt} scheme. The enum declaration and encoding rules are defined
    in Section~\ref{sec:enums}.
  ...
  \label{sec:enums}
    ...
    \subsubsection{Encoding}
      ...
      \texttt{VarUInt} encoding (Section~\ref{sec:length-encoding}). The encoded
      numeric value is the declared discriminant of the enumeration case.
...
@bar("a", "b")
    ...
    Annotation arguments, if present, MUST be string literals. Whitespace around
    parentheses and commas is permitted. An empty argument list (e.g.,
    \texttt{@foo()}) and an omitted argument list (e.g., \texttt{@foo}) are
    syntactically distinct but MUST be treated equivalently by implementations.
    ...
    text (for example, a deprecation reason or replacement hint).
  ...
  \subsubsection{Methods}
    ...
    tuple is not a first-class type in arf source; it is a serialization
    construct that encodes “zero or more ordered values” as a single payload.
...
\]
    ...
    \subsubsection{Structs}
      ...
      \begin{enumerate}
        ...
        \item For a field whose declared type is not \texttt{optional<\dots>}:
          ...
          the rules for its type.  Encoders \emph{MUST NOT} omit such fields;
  ...
  \label{sec:unary-tuples}
    ...
    single payload.  Tuples are not declared in arf source; they are derived
...
\]
    ...
    \texttt{VarUInt(0)} followed by zero bytes.  This representation MAY be
    used by future extensions but is distinct from the case where the
    method declares no unary inputs or outputs at all (see
    Section~\ref{sec:requests} and Section~\ref{sec:unary-output}).
    ...
    extended by appending unary inputs or unary outputs in a later schema
  ...
  \label{sec:metadata}
    ...
    Metadata are out-of-band, key--value annotations attached to an RPC
    request or response.  Metadata are carried only on the wire and do not
    appear in arf source files, the IDL grammar, or the ABNF.  This
    specification does not define any standard metadata keys or values; all
    semantics are application-defined.

    A metadata block is a length-delimited sequence of zero or more metadata
    entries.  It is encoded as:

    \begin{enumerate}
      \item \texttt{VarUInt(L)}, where $L$ is the number of bytes in the
        metadata body that follows; and
      \item exactly $L$ bytes forming the metadata body.
    \end{enumerate}

    The metadata body is itself a concatenation of zero or more \emph{metadata
    entries}.  Each entry consists of:

    \begin{enumerate}
      \item \texttt{VarUInt(klen)}, the length in bytes of the key;
      \item \texttt{klen} bytes of key data; and
      \item \texttt{VarUInt(vlen)}, the length in bytes of the value; and
      \item \texttt{vlen} bytes of value data.
    \end{enumerate}

    Keys are encoded as raw octets but MUST be restricted to the following
    ASCII character set:

    \begin{itemize}
      \item lowercase letters \texttt{a}--\texttt{z},
      \item digits \texttt{0}--\texttt{9},
      \item hyphen \texttt{-}, underscore \texttt{\_}, and period \texttt{.}.
    \end{itemize}

    The key length \texttt{klen} MUST be greater than zero.  Implementations
    MUST reject metadata blocks that contain keys with invalid characters or
    zero-length keys.

    Values are arbitrary binary payloads.  No encoding, structure, or
    interpretation is imposed by this specification.  Implementations MAY
    treat value bytes as opaque and MAY choose to ignore some or all
    metadata entries.

    Keys MAY appear multiple times within the same metadata block.  The
    ordering of entries, including duplicated keys, MUST be preserved by
    the encoder and decoder.  The meaning of duplicate keys, if any, is
    entirely application-defined and outside the scope of this
    specification.

    A metadata block with \texttt{L = 0} encodes an empty metadata set and
    occupies a single \texttt{VarUInt(0)} byte on the wire.  Endpoints that
    do not wish to attach metadata MUST still emit and consume the metadata
    length prefix, and MAY treat the metadata body as opaque and skip it
    using the length prefix.

    Metadata blocks are carried only in the payloads of \texttt{INVOKE} and
    \texttt{RESPONSE} frames in this version of the protocol.  Streaming
    frames (\texttt{IN\_STREAM}, \texttt{IN\_CLOSE}, \texttt{OUT\_STREAM},
    \texttt{OUT\_CLOSE}), \texttt{CANCEL} frames, and \texttt{ERROR} frames
    MUST NOT contain metadata blocks.
  ...
  \label{sec:optional-fields}
    ...
    \begin{itemize}
      ...
      \item if the presence byte is \texttt{0x01}, the serialized
            representation of \texttt{T}, encoded according to the rules
            for type \texttt{T}.
    \end{itemize}
    ...
    The type parameter \texttt{T} MAY be any valid arf type, including
    ...
    declaration order.  The struct length prefix continues to delimit the
    entire struct body, allowing receivers to skip trailing fields safely.
...
struct User {
    ...
    \begin{itemize}
      ...
      \item It treats the remaining bytes in the struct body as unknown data and skips
        them using the length prefix.
    \end{itemize}

    Because all struct values are length-delimited, receivers never need to understand
    the internal layout of unknown fields in order to skip them safely.  This guarantees
    forward compatibility for nested structs, arrays of structs, and arbitrarily complex
    type graphs.
...
\label{sec:rpc-protocol}
  ...
  \label{sec:frame-format}
	...
	different frames MUST NOT be interleaved on the wire.  A single frame MAY be
	split across multiple transport segments, but the concatenation of bytes
	delivered by the transport for that connection MUST form a well-formed
	sequence of complete frames.  The format of a frame is:
      ...
      \begin{center}
        \begin{tabular}{@{}ll@{}}
          \toprule
            ...
            0x07  & \texttt{ERROR}: terminal error (Section~\ref{sec:error-frames}). \\
    ...
    bytes. Examples of suitable transports include TCP connections, QUIC streams,
    ...
    \label{sec:multiplexing}
      ...
      \subsubsection{Correlation Identifiers}
        ...
        inactive \texttt{CorrelationID} as a protocol error, \emph{except}
        for \texttt{CANCEL} frames. A \texttt{CorrelationID} is considered
        inactive if and only if the completion conditions in
        Section~\ref{sec:rpc-completion} have been satisfied for that
        identifier.

        A \texttt{CANCEL} frame received for an unknown or inactive
        \texttt{CorrelationID} (as defined in Section~\ref{sec:rpc-completion})
        MUST be silently ignored and MUST NOT be treated as a protocol
        violation.
      ...
      \subsubsection{Concurrent RPCs}
        ...
        \begin{itemize}
          ...
          \item Exactly one \texttt{INVOKE} frame MUST appear, and it MUST be
            the first frame for that \texttt{CorrelationID}. A second
            \texttt{INVOKE} for an active \texttt{CorrelationID} is a
            connection-level protocol error (see Section~\ref{sec:requests}
            and Section~\ref{sec:error-frames}).
        \end{itemize}
...
FNV-1a-32(input):
  ...
  Return h.
    ...
    \subsubsection{Identifier Invariance Across Frames}
    \label{sec:identifier-invariance}

      For a given \texttt{CorrelationID}, the tuple
      (\texttt{PackageID}, \texttt{ServiceID}, \texttt{MethodID}) defines the
      identity of the target method and MUST remain constant across all frames
      belonging to that RPC.

      Specifically:

      \begin{itemize}
        \item The \texttt{INVOKE} frame establishes the authoritative tuple of
          identifiers for the RPC.
        \item Every subsequent frame with the same \texttt{CorrelationID}
          (\texttt{IN\_STREAM}, \texttt{OUT\_STREAM}, \texttt{RESPONSE},
          \texttt{CANCEL}, \texttt{ERROR}, etc.) MUST carry the exact same
          identifier values.
        \item Receivers MUST treat any frame whose identifiers do not exactly
          match the established tuple for the corresponding
          \texttt{CorrelationID} as a protocol error.
      \end{itemize}
  ...
  \label{sec:requests}
    ...
    \begin{itemize}
      \item The payload of every \texttt{INVOKE} frame MUST begin with exactly
        one metadata block encoded as described in Section~\ref{sec:metadata}.

      \item If \textbf{HasInput} is \texttt{true}, the metadata block MUST be
        immediately followed by the serialization of the unary input tuple.

      \item If \textbf{HasInput} is \texttt{false}, the payload MUST consist
      solely of the metadata block. An empty metadata set is encoded as a
      metadata block of length zero.

      \item If \textbf{HasInputStream} is \texttt{true}, the input stream is
        considered open immediately after the \texttt{INVOKE} frame.
        The client MAY send zero or more \texttt{IN\_STREAM} frames.
        If the RPC completes normally (i.e., without cancellation or error),
        the client MUST send exactly one \texttt{IN\_CLOSE}.

        If the RPC is aborted by sending \texttt{CANCEL}, the client MUST NOT
        send \texttt{IN\_CLOSE}. Cancellation implicitly closes the input stream.
    \end{itemize}

    The unary input tuple is encoded as:

    \begin{enumerate}
      \item \texttt{VarUInt(L)}, where $L$ is the total byte length of
        the tuple payload.
      \item The serialized representation of each unary input value, in
        declaration order.
    \end{enumerate}
    ...
    Upon receiving an \texttt{INVOKE} frame whose \texttt{CorrelationID}
    identifies an RPC that it still considers active (as defined in
    Section~\ref{sec:rpc-completion}), the server MUST treat this as a
    connection-level protocol error. The server MUST NOT send an
    \texttt{ERROR} frame for that \texttt{CorrelationID} in response, and
    SHOULD close the underlying transport connection after logging or
    otherwise recording the violation.
    ...
    \label{RPC-MethodForms}
      ...
      \begin{table}[H]
        \centering
        \caption{arf Method Forms and Permitted Streaming Frames \\ (Successful Completion Only; Cancellation Overrides)}
      ...
      \begin{itemize}
        \item Exactly one \texttt{INVOKE} frame MUST be sent, and it MUST be
          the first frame.
        \item If HasInS is \texttt{Y} and the RPC completes successfully, the
          client MAY send zero or more \texttt{IN\_STREAM} frames followed by
          exactly one \texttt{IN\_CLOSE}. If the RPC is terminated by
          \texttt{CANCEL} or \texttt{ERROR}, \texttt{IN\_CLOSE} MUST NOT be
          sent. If HasInS is \texttt{N}, the client MUST NOT send
          \texttt{IN\_STREAM} or \texttt{IN\_CLOSE} at any time.

        \item If HasOutS is \texttt{Y} and the RPC completes successfully, the
          server MAY send zero or more \texttt{OUT\_STREAM} frames followed by
          exactly one \texttt{OUT\_CLOSE}. If the RPC is terminated by
          \texttt{CANCEL} or \texttt{ERROR}, \texttt{OUT\_CLOSE} MUST NOT be
          sent. If HasOutS is \texttt{N}, the server MUST NOT send
          \texttt{OUT\_STREAM} or \texttt{OUT\_CLOSE} at any time.
      \end{itemize}
    ...
    \label{sec:rpc-completion}
      ...
      it terminates via \texttt{RESPONSE} and all required stream closures have
      been observed for that RPC, as defined below.

      Any RPC whose lifecycle includes a terminal \texttt{ERROR} frame is
      considered to have completed abnormally.

      For an RPC that is still active at the server, client-initiated
      cancellation necessarily results in abnormal completion: upon receiving a
      \texttt{CANCEL} frame for an active RPC, the server MUST terminate the RPC
      and send exactly one terminal \texttt{ERROR} acknowledgement for that
      \texttt{CorrelationID} (Section~\ref{sec:cancellation}). A
      \texttt{CANCEL} received after the server has already considered the RPC
      complete is a late control signal and does not change the completion
      state or produce an additional \texttt{ERROR} frame.

      For a given \texttt{CorrelationID}, an RPC is considered complete, from the
      point of view of a given endpoint, when that endpoint has observed one of
      the following terminal conditions for that endpoint:

      \begin{itemize}
        \item The endpoint has sent or received an \texttt{ERROR} frame for that
          \texttt{CorrelationID}; or

        \item The endpoint has observed both:
          \begin{itemize}
            \item successful unary completion; and
            \item closure of every stream flowing \emph{toward} that endpoint.
          \end{itemize}
      \end{itemize}
      ...
      Thus, for a \emph{successful} RPC (no \texttt{ERROR} frame), the client
      considers the RPC complete when:

      \begin{itemize}
        \item it has received \texttt{RESPONSE}, and
        \item if \textbf{HasOutputStream} is \texttt{true}, it has received
          \texttt{OUT\_CLOSE}, and
        \item if \textbf{HasInputStream} is \texttt{true}, it has sent
          \texttt{IN\_CLOSE}.
      \end{itemize}
      ...
      \begin{itemize}
        \item it has sent \texttt{RESPONSE}, and
        \item if \textbf{HasInputStream} is \texttt{true}, it has received
          \texttt{IN\_CLOSE}, and
        \item if \textbf{HasOutputStream} is \texttt{true}, it has sent
          \texttt{OUT\_CLOSE}.
      \end{itemize}
      ...
      \texttt{CorrelationID} MUST be treated as a protocol error and SHOULD
      ...
      connection until it has either observed a terminal condition for the
      RPC (an \texttt{ERROR} frame or the appropriate combination of
      \texttt{RESPONSE} and stream closures) or closed the underlying
  ...
  \subsection{Interleaving of Input and Output Streams}
    ...
    The server MUST transmit the \texttt{RESPONSE} frame before sending any
    \texttt{OUT\_STREAM} or \texttt{OUT\_CLOSE} frames, as specified in
    Section~\ref{sec:unary-output}. Input and output streams MAY otherwise be
    arbitrarily interleaved after transmission of \texttt{RESPONSE}.

  \subsection{Unary vs Streaming Semantics}

    \subsubsection{Unary Input}
      If \textbf{HasInput} is \texttt{true}, the payload of the
      \texttt{INVOKE} frame MUST consist of:

      \begin{itemize}
        \item a metadata block encoded as specified in
          Section~\ref{sec:metadata}; followed immediately by
        \item the serialized unary input tuple, encoded as specified in
          Section~\ref{sec:unary-tuples}.
      \end{itemize}

      If \textbf{HasInput} is \texttt{false}, the payload of the
      \texttt{INVOKE} frame MUST consist solely of the metadata block
      described in Section~\ref{sec:requests}. An empty metadata set is
      encoded as a metadata block of length zero.

      In all cases, an \texttt{INVOKE} payload MUST begin with exactly one
      metadata block. When \textbf{HasInput} is \texttt{false}, the client
      MUST NOT encode a unary input tuple.
    ...
    \label{sec:unary-output}
      ...
      The payload of the \texttt{RESPONSE} frame consists of a metadata block
      followed, optionally, by a unary output tuple.

      If \textbf{HasOutput} is \texttt{true}, the payload of the
      \texttt{RESPONSE} frame MUST contain:

      \begin{itemize}
        \item a metadata block encoded as specified in
          Section~\ref{sec:metadata}; and
        \item the serialized unary output tuple, encoded as specified in
          Section~\ref{sec:unary-tuples}, using the unary output values in
          declaration order.
      \end{itemize}

      If \textbf{HasOutput} is \texttt{false}, the payload of the
      \texttt{RESPONSE} frame MUST contain a metadata block only.  An empty
      metadata set is encoded as a metadata block of length zero.

      The transmission of a \texttt{RESPONSE} frame does not imply closure
      of any input or output streams. Unary output and stream lifetimes
      are independent.
    ...
    \subsubsection{Output Stream}
      ...
      \begin{itemize}
        ...
        \item If the RPC completes successfully, the server MUST send exactly
          one \texttt{OUT\_CLOSE} frame.
...
IN_CLOSE(CID)
      ...
      \textbf{Note:} This example demonstrates that unary completion does not
      imply closure of the input stream. The server may legally return before
      the client finishes sending input elements.
    ...
    \label{sec:error-frames}
      ...
      \begin{itemize}
        \item At most one \texttt{ERROR} frame MAY be sent per endpoint for a
          given RPC.
        \item Once an endpoint has sent an \texttt{ERROR} frame, it MUST NOT send
          any further frames with that \texttt{CorrelationID}.
      ...
      The payload of an \texttt{ERROR} frame is defined as follows:

      \begin{itemize}
        \item If \texttt{PayloadLength} is zero, the \texttt{ERROR} frame carries
          no structured payload.  The reason for termination is implementation-
          defined and MAY be inferred from local context only.
        \item If \texttt{PayloadLength} is non-zero, the payload MUST consist of
          the serialized representation of exactly one \emph{RPC error
          descriptor}, encoded according to Section~\ref{sec:serialization}.
      \end{itemize}
...
struct RPCError {
    ...
    details optional<bytes>;
      ...
      \begin{itemize}
        \item \texttt{code} is a numeric error code.  This specification reserves
          the following well-known values:

          \begin{itemize}
            \item \texttt{0}: \textbf{OK}.  The RPC terminated without
              application-level error.  This code is intended primarily for use
              with cancellation acknowledgements (see
              Section~\ref{sec:cancellation}).
            \item \texttt{1}: \textbf{CANCELLED}.  The RPC was terminated because
              the client requested cancellation.
            \item \texttt{2}: \textbf{UNKNOWN}.  An unspecified server-side
              failure occurred.
          \end{itemize}
        ...
        \item \texttt{message} is a human-readable diagnostic string intended for
          logging and debugging.  It MUST NOT be relied on for programmatic
          behavior.
      ...
      treated as connection-level protocol errors: implementations SHOULD
    ...
    \label{sec:cancellation}
      ...
      same \texttt{CorrelationID} as the active call.  A \texttt{CANCEL} frame
      MUST carry an empty payload; its \texttt{PayloadLength} field MUST be zero.
      ...
      \begin{itemize}
        \item MUST cease processing the RPC and release any associated resources
          as soon as practical;
        \item MUST NOT send any further frames related to that
          \texttt{CorrelationID}, \emph{except} as permitted below;
        \item MUST send exactly one terminal \texttt{ERROR} frame for that
          \texttt{CorrelationID}, as defined in Section~\ref{sec:error-frames};
          and
        \item MUST send no further frames after that \texttt{ERROR}.
      \end{itemize}
      ...
      any additional frames (including \texttt{ERROR}) in response, and MUST
      NOT treat the late \texttt{CANCEL} as a protocol error solely by virtue
      of its arrival after completion.

      When acknowledging a client-initiated cancellation that did not surface an
      application-level failure, the server SHOULD use an \texttt{ERROR} payload
      whose \texttt{RPCError.code} is either:

      \begin{itemize}
        \item \texttt{0} (\textbf{OK}), indicating that the RPC was cancelled at
          the client's request and no error occurred; or
        \item \texttt{1} (\textbf{CANCELLED}), indicating that the RPC was
          terminated due to client cancellation and that the server did not
          complete normal processing.
      \end{itemize}

      The choice between \texttt{OK} and \texttt{CANCELLED} is a policy decision
      for the implementation.  In both cases, the \texttt{ERROR} frame serves as
      a definitive acknowledgement that the server has observed and acted upon
      the \texttt{CANCEL}.

      A client that sends \texttt{CANCEL}:

      \begin{itemize}
        \item MUST NOT send any frames other than the \texttt{CANCEL} itself for
            that \texttt{CorrelationID} after transmitting \texttt{CANCEL};
        \item MAY treat the RPC as terminated for local application purposes
          immediately after sending \texttt{CANCEL} (for example, by releasing
          local resources or failing any associated futures or promises);
        ...
        \item SHOULD ignore any subsequently received application-level frames
          other than the terminal \texttt{ERROR} acknowledgement, if it chooses
          to continue reading from the connection.
      \end{itemize}
      ...
      Sending \texttt{CANCEL} implicitly closes the input stream and/or output
      stream, if any are currently open. No \texttt{IN\_CLOSE} or
...
static inline int64_t arf_zigzag64_decode(uint64_t z) {
  ...
  \label{annex:impl-notes}
    ...
    \subsection{Error Mapping and Diagnostics}
      ...
      via the \texttt{RPCError} descriptor (Section~\ref{sec:error-frames}). This
      structure provides a machine-readable error code, a human-readable message,
      and optional opaque details.
      ...
      Implementations SHOULD use the \texttt{RPCError.details} field sparingly and
      only for optional, diagnostic data that is safe to expose. This field MAY
      carry structured, application-specific error information encoded out of
      band, but no interoperability is implied unless both peers explicitly
      agree on its format.

      Error messages are intended for human diagnostics and logging only and MUST
      NOT be relied upon for programmatic behavior.
